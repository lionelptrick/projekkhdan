...
if(isset($_POST['search'])){
 ...
 if(!empty($input_a)){
  $q = "WHERE aktiviti.jenisaktiviti LIKE '%$input_a%' ";
 }
}  ?>
<h2 class='jenisaktiviti'>Semua Aktiviti</h2>
...
<div class='row'>
<?php
 # Dapatkan semua aktiviti atau berdasarkan carian ahli
 ...
 if(mysqli_num_rows($result) > 0){
  ...
  while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
    ...
    echo "<tr>
      <td>$counter</td>
      <td>
        <b>$jenisaktiviti</b> <br><br>Masa: $masa <br>
        Lokasi: $lokasi<br> Ganjaran: $mata $label_mata<br><br>
        <a class='button' href='papar_aktiviti.php?id=$id'>Lihat</a>
        <a class='button' href='sertai_aktiviti.php?id=$id&action=add'>Sertai</a>
       </td>
       <td>$img</td>
      </tr>";
      $counter += 1;
